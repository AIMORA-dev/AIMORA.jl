...
function initialize_step_context(
    ...
    context = EMTStepContext(
        ...
        Int[],
        Float64[],
        Vector{Int}(undef, system.node_count + 1),
        SparseNodeGroupSolveWorkspace(system.node_count),
        String(source),
        dt_s,
        t_end_s,
        0.0,
        ...
        Vector{Float64}(undef, sample_count),
        Matrix{Float64}(undef, system.node_count, sample_count),
...
function _solve_grouped_steady_state_admittance(
    ...
    active_nodes = sort(unique(Int.(representatives)))
    length(active_nodes) == node_count &&
        return _solve_steady_state_linear_system(admittance, rhs)
    ...
    reduced_solution =
        _solve_steady_state_linear_system(reduced_admittance, reduced_rhs)
    solution = Vector{ComplexF64}(undef, node_count)
    for node in 1:node_count
        solution[node] = reduced_solution[reduced_index[Int(representatives[node])]]
    end
    return solution
end
...
function _solve_steady_state_linear_system(
    ...
    try
        return admittance \ rhs
    catch error
        error isa SingularException || rethrow()
    end
    solution = qr(admittance, ColumnNorm()) \ rhs
    residual = norm(admittance * solution - rhs, Inf)
    residual_scale = max(norm(rhs, Inf), 1.0)
    residual <= 1.0e-10 * residual_scale || throw(ArgumentError(
...
function _deck_external_steady_state_voltage_phasors(
    ...
    node_voltage_phasors = _solve_grouped_steady_state_admittance(
        admittance,
        rhs,
        switch_representatives,
    )
...
function _primary_dynamic_timestep_state(
    ...
    switches = max(1, Int(switch_count))
